...
class MerkleTools {
    constructor(treeOptions = { hashType: 'sha256' }) {
        this.tree = {
            isReady: false,
            leaves: [],
            levels: []
     ...
     * @memberof MerkleTools
     */
    addLeaves(valuesArray, doHash = false) {
    ...
    getProof(index) {
        ...
        for (let x = currentRowIndex; x > 0; x--) {
            ...
            if (index === currentLevelNodeCount - 1 &&
     ...
     * @memberof MerkleTools
     */
    validateProof(proof, targetHash, merkleRoot, doubleHash = false) {
        ...
        for (const leaf of proof) {
            if (leaf.left) {
                ...
                if (doubleHash) {
                    proofHash = this.hashFunction(this.hashFunction(Buffer.concat([this.getBuffer(leaf.left), proofHash])));
                }
                else {
                    proofHash = this.hashFunction(Buffer.concat([this.getBuffer(leaf.left), proofHash]));
                }
            }
            else if (leaf.right) {
                // then the sibling is a right node
                if (doubleHash) {
                    proofHash = this.hashFunction(this.hashFunction(Buffer.concat([proofHash, this.getBuffer(leaf.right)])));
                }
                else {
                    proofHash = this.hashFunction(Buffer.concat([proofHash, this.getBuffer(leaf.right)]));
                }
            }
            else {
    ...
    hashFunction(value) {
        switch (this.hashType) {
            case 'SHA3-224':
                return Buffer.from(js_sha3_1.sha3_224.update(value).array());
            case 'SHA3-256':
                return Buffer.from(js_sha3_1.sha3_256.update(value).array());
            case 'SHA3-384':
                return Buffer.from(js_sha3_1.sha3_384.update(value).array());
            case 'SHA3-512':
                return Buffer.from(js_sha3_1.sha3_512.update(value).array());
            default:
     ...
     * @memberof MerkleTools
     */
    calculateNextLevel(doubleHash = false) {
        const nodes = [];
     ...
     * @memberof MerkleTools
     */
    calculateBTCNextLevel(doubleHash = false) {
        const nodes = [];
        const topLevel = this.tree.levels[0];
        const topLevelCount = topLevel.length;
...
exports.default = MerkleTools;
//# sourceMappingURL=merkletools.js.map